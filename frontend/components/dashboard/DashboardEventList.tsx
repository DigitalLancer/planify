...
export default function DashboardEventList({ events }: { events: Event[] }) {
    ...
    const firstFiveEvent = events.slice(startIndex, startIndex + 5);
    ...
    return (
        <div className='mt-3 flex flex-col gap-5 relative pl-4'>
            ...
            {firstFiveEvent && firstFiveEvent.length > 0 ? (
                firstFiveEvent.map((event, index) => (
                    <DashboardEventListItem data={event} key={event.id} index={index}/>
                ))
            ) : (
                <span className='italic opacity-60'>No event to display</span>
            )}
        </div>
    )
}
